...
Dragon::Dragon(ObjectSpawnInfo objectSpawnInfo)
	//: m_attackTimer{2}
{
	...
	// set object collision component and check spawn location
...
void Dragon::Tick(float deltaTime)
{
	CheckHealthLimit();
...
void Dragon::HandleBeginOverlap(ObjectBase* pOtherCollider)
{
...
void Dragon::Render(SDL_Renderer* pRenderer)
{
	m_pAnimationComponent->Render(pRenderer, GetX(), GetY(), GetWidth(), GetHeight(), 0.f);
}

void Dragon::PositionUpdate(float deltaTime)
{
	...
	if (!canMove && (m_pObjectStateMachine->GetCurrentState()->GetStateType() != BaseState::StateType::kAttack))
	{
		ChangeDirection();
	}
...
void Dragon::CreateAndInitStateMachine()
{
